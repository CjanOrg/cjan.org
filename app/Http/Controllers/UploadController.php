	...
	public function postResults(Request $request)
	{
		...
		if ($json && strlen(trim($json)) > 0)
		{
			...
			try {
				...
				try
				{
					$projectArtifactId = $projectArtifactId->toArray();
					$projectGroupId = $projectGroupId->toArray();

					$id = $projectArtifactId['id'];
					$artifactId = $projectArtifactId['name'];

					$groupId = $projectGroupId['name'];
					$projectArtifactId['project_group_id'] = $projectGroupId;
